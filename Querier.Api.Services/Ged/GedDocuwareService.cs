...
namespace Querier.Api.Services.Ged
{
    public class GedDocuwareService : IQFileReadOnlyDeposit
    {
        ...
        private QFileDeposit GetInformationFromDB()
        {
            ...
            {
                return apidbContext.HAFileDeposit.FirstOrDefault(r => r.Type == TypeFileDepositEnum.Docuware);
            }
        } 
        ...
        public async Task<FillFileInformationResponse> FillFileInformations()
        {
            ...
            using (var apidbContext = _contextFactory.CreateDbContext())
            {
                ...
                {
                    apidbContext.HAFilesFromFileDeposit.AddRange(missingRecords);
        ...
        public async Task<List<GetInformationsResponse>> GetSpecificInformation(List<GetSpecificInformationRequest> variablesFilter)
        {
            ...
            using (var apidbContext = _contextFactory.CreateDbContext())
            {
                ...
                {
                    var doc = apidbContext.HAFilesFromFileDeposit.FirstOrDefault(d => d.FileRef == document.Id.ToString());
                    if(doc != null)
                    {
                        response.Add(new GetInformationsResponse { FilePath = document.Title, IdHAFilesFromFileDeposit = doc.Id });
                    }  
                }
            }
            return response;
